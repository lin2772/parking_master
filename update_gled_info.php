<?php
	...
	$sql_update_capacity = "UPDATE garage_level_event_date SET capacityallocated = $capacity where (garageid = $GARAGEID AND levelnum = $LEVELNUM AND eventid = $EVENTID AND date = $DATE)";
	if(mysqli_query($con, $sql_update_capacity)){
		echo "Capacity updated successful";
	}else{
		echo "Capacity update fail";
	}
